...
MLPModelWrapper::MLPModelWrapper(const std::vector<uint>& npl)
{
    this->_model = createMlpModel(npl.data(), npl.size());
    this->_inputDim = this->_model->npl[0];
    this->_outputDim = this->_model->npl[this->_model->L];
}
...
double MLPModelWrapper::evaluate(const m2& inputs, const m2& outputs) const
{
    ...
    const double result = evaluateModelAccuracyClassification(this->_model, inputArr, outputArr, inputs.size(), this->_inputDim, this->_outputDim) * 100;
...
void MLPModelWrapper::train(const m2& inputs, const m2& outputs, const double alpha, const uint epochs) const
{
    ...
    trainMlpModelClassification(this->_model, inputArr, outputArr, inputs.size(), this->_inputDim, this->_outputDim, alpha, epochs);
