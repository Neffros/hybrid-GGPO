...
int
HybridInputQueue::GetLastConfirmedFrame()
{
...
int
HybridInputQueue::GetFirstIncorrectFrame()
{
	return _first_incorrect_frame;
}

void
HybridInputQueue::DiscardConfirmedFrames(int frame)
{
	ASSERT(frame >= 0);
...
void
HybridInputQueue::ResetPrediction(int frame)
{
...
bool
HybridInputQueue::GetConfirmedInput(int requested_frame, GameInput* input)
{
...
HybridInputQueue::GetInput(int requested_frame, GameInput* input, void* values, int player, int size)
{
	...
	if (_prediction.frame == GameInput::NullFrame) {
		...
		_prediction.frame = requested_frame;
	}

	Log("basing new prediction frame on user strategy.\n");

	if (this->_serviceProvider->getInputPredictionStrategyService() != NULL)
		this->_serviceProvider->getInputPredictionStrategyService()->predict(requested_frame, values, size, player);

	int offset = player * size;
	char* bits = (char*)values;
...
void
HybridInputQueue::AddInput(GameInput& input)
{
	int new_frame;
...
void
HybridInputQueue::AddDelayedInputToQueue(GameInput& input, int frame_number)
{
	...
	// TD: Manage a list of predicted inputs since we can now have multiple predictions during desync
	if (_prediction.frame != GameInput::NullFrame) {
...
int
HybridInputQueue::AdvanceQueueHead(int frame)
{
...
void
HybridInputQueue::Log(const char* fmt, ...)
{
	char buf[1024];
	size_t offset;
	va_list args;
